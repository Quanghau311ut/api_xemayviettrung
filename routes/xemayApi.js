...
router.post("/edit/:id_xe", async function (req, res) {
    ...
    if (
        !req.body ||
        !req.body.id_thuong_hieu ||
        !req.body.id_danh_muc ||
        !req.body.ten_xe ||
        !req.body.model ||
        !req.body.mau_sac ||
        !req.body.nam_san_xuat ||
        !req.body.gia ||
        !req.body.so_luong ||
        !req.body.anh_dai_dien ||
        !req.body.khuyen_mai
    ) {
        return res.status(400).send("Yêu cầu thiếu thông tin");
    }
    ...
    try {
        ...
        await connection.execute(
            "UPDATE quanlyxemay SET id_thuong_hieu = ?, id_danh_muc = ?, ten_xe = ?, model = ?, mau_sac = ?, nam_san_xuat = ?, gia = ?, so_luong = ?,anh_dai_dien = ?,khuyen_mai = ?, updated_at = CURRENT_TIMESTAMP WHERE id_xe = ?",
            [
                req.body.id_thuong_hieu,
                req.body.id_danh_muc,
                req.body.ten_xe,
        ...
        if (thongSoKyThuat) {
            ...
            await connection.execute(
                ...
                [
                    thongSoKyThuat.dung_tich_xilanh,
                    thongSoKyThuat.cong_suat_toi_da,
                    thongSoKyThuat.momen_xoan_toi_da,
                    thongSoKyThuat.tieu_hao_nhien_lieu,
                    thongSoKyThuat.hop_so,
                    thongSoKyThuat.trong_luong,
                    id_xe,
                ]
            );
        ...
        if (anhChiTiet) {
            ...
            await connection.execute("DELETE FROM anhchitiet WHERE id_xe = ?", [
                id_xe,
            ]);
